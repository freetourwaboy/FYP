...
function generateCode128(code) {
  if (checkAorC(code) == 'A') {
      let total = 0;
      for (let i = 0; i < code.length; i++) {
          total += ((i + 1) * getCode128_AValuedec(code[i]));
      }
      ...
      barcode += getCode128_AValuebin(getCode128_AChar(checksum));
      barcode += '1100011101011';
      console.log(barcode);
      return {barcode, fullcode};
  }
  else  if (checkAorC(code) == 'C') {
    let total = 105;
    let barcode = '11010011100';
    if (code.length%2==0)
      var temp_length = code.length;
    else 
      var temp_length =code.length-1;
    
    let j=1;

    for (let i = 0; i < temp_length; i+=2) {
      let pair = code[i]+code[i+1];
      barcode += getCode128_CValuebin(pair);

     // barcode+=' ';
     console.log(total);
      total += (j * getCode128_CValuedec(pair));
      j++;
  }

    if (temp_length!=code.length)
    {
      total +=(j * 101)
      j++;
      total += (j * getCode128_AValuedec(code[temp_length]));
      barcode += getCode128_CValuebin('CODEA');

      barcode += getCode128_AValuebin(code[temp_length]);
    }
      let checksum = total % 103;
      barcode += getCode128_CValuebin(getCode128_CChar(checksum));
      console.log(getCode128_CValuebin(getCode128_CChar(checksum)));
...
// 11010011100 10110011100 10010110000 11101100010 1100011101011
// 11010011100 10110011100 10010110000 11110101110 1100011101011
